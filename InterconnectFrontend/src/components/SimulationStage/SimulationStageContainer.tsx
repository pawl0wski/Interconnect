...
const SimulationStageContainer = () => {
    ...
    const handleOnClick = useCallback((e: KonvaEventObject<MouseEvent>) => {
        if (e.evt.button !== 0) {
    ...
    const handleOnContextMenu = (e: KonvaEventObject<PointerEvent>) => {
        const name = SimulationStageEntitiesUtils.getTargetOrParentEntityInfo(e.target as ObjectWithName);
        ...
        simulationStageContextMenuStore.setCurrentContextMenu(entity.type, entity.id, { x, y });
    };

    return <SimulationStage onClick={handleOnClick}
                            showPlacementCursor={Boolean(entityPlacementStore.currentEntityType)}
