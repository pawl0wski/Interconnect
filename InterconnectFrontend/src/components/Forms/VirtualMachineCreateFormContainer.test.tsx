...
describe("VirtualMachineCreateFormContainer", () => {
    test("should show empty values validation errors", async () => {
        ...
        expect(screen.getByText("Nazwa nie może być pusta")).toBeInTheDocument();
        expect(screen.getByText("Musisz wybrać przynajmniej jedno wirtualne CPU")).toBeInTheDocument();
        expect(screen.getByText("Musisz zadeklarować pamięć operacyjną dla maszyny wirtualnej")).toBeInTheDocument();
        expect(screen.getByText("Musisz wybrać obraz dla maszyny wirtualnej")).toBeInTheDocument();
    });
});